...
k= 1./g;
k_flat = reshape(k.',1,[]); % flat to 1xN тN程
...
% [i,j] = find(k == k_flat(index),1,'last'); % 眖т縩だ兜计ゑ耕兜
...
str3 = '.^2).* abs(1+';
% ------------------------------------------------------%
% ﹚竡拔ㄧ计倒 -> integral(縩だ拔ㄧ计秸ノ,-inf,inf)
% a(r), r = t./(n+1), t = r(n+1) 
...
for k = 1:iter % with a(k), g_struct.varing_gain(k)
    % ar(r, g_struct.samples, k ,g_struct.varing_gain) % k is the そΑい j
